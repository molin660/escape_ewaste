...
orangefish=image.load('leftorangefish.png')
...
def computer_positions(comp_list):
        for idx, comp_pos in enumerate(comp_list):
                if comp_list[1] >= 0 and comp_pos[1] < HEIGHT:
                        comp_pos[1] += 10
                else:
                        comp_list.pop(idx)
                        score+=1
        return score
def comp_col_check(comp_list, score):
        for comp_pos in comp_list:
                if comp_check(comp_list, player_pos):
                        return True
        return False
def comp_col_det(comp_li):
        p_x= player_pos[1]
...
while running:
        for evnt in event.get():
                ...
                if Second>10 and Second%15==0 or Second>=57:
                        MOVABLE=False
